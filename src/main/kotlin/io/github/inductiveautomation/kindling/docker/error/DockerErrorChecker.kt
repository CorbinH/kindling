 ...
 */
private fun gwbkRestoreBindings(services: List<DockerServiceModel>): List<Triple<DockerServiceModel, String, String>> = services.mapNotNull { model ->
...
private object DuplicateHostnamesChecker : DockerErrorChecker {
    override fun check(services: List<AbstractDockerServiceNode<*>>, baseDir: Path?): List<DockerError> = services
...
private object DuplicateCliFlagsChecker : DockerErrorChecker {
    override fun check(services: List<AbstractDockerServiceNode<*>>, baseDir: Path?): List<DockerError> = services.flatMap { svc ->
        svc.model.commands
...
private object MissingGwbkFileChecker : DockerErrorChecker {
    override fun check(services: List<AbstractDockerServiceNode<*>>, baseDir: Path?): List<DockerError> = gwbkRestoreBindings(services.map(AbstractDockerServiceNode<*>::model)).mapNotNull { (svc, _, bindPath) ->
