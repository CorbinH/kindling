...
import io.github.inductiveautomation.kindling.docker.model.compose.Deploy.EndpointMode
import io.github.inductiveautomation.kindling.docker.model.compose.Deploy.Mode
import io.github.inductiveautomation.kindling.docker.model.compose.Deploy.RestartPolicy.Condition
...
class DeployEditor(data: Deploy) : ComposeObjectEditor<Deploy>("Deploy", data) {
    val mode by combo(options = Mode.entries, initialValue = data.mode) {
        data.mode = it
    }

    val replicas by numeric(initialValue = data.replicas) { data.replicas = it ?: 1 }

    val endpointMode by combo("Endpoint Mode", EndpointMode.entries, data.endpointMode) {
    ...
    inner class RestartPolicyEditor(
        ...
        val condition by combo(options = Condition.entries, initialValue = data.condition) {
            data.condition = it
        }
