 ...
 */
class IgnitionNodeConnector(
    fromGateway: IgnitionServiceNode,
    val index: Int,
    private val canvas: Canvas,
) : JComponent() {
    var from: IgnitionServiceNode = fromGateway
    lateinit var to: IgnitionServiceNode
    ...
    private val mouseObserver = Timer(10) {
        if (!connectionInProgress) {
            SwingUtilities.invokeLater {
                this@IgnitionNodeConnector.finalizeConnection()
            }
            return@Timer
        }
            ...
            mouseMoved {
                ...
                if (contains != highlightPath) {
                    ...
                    val parentEvent: MouseEvent = SwingUtilities.convertMouseEvent(
                        this@IgnitionNodeConnector,
                        it,
                        this@IgnitionNodeConnector.parent,
                    )
    ...
    private val hostNameChangeListener = ServiceModelChangeListener {
        ...
        if (to.model.hostName != null && to.model.hostName != from.model.environment[hostNameEnvVar]) {
            ...
            from.model.fireServiceModelChangedEvent()
        }
    }
    ...
    private fun finalizeConnection() {
        ...
        from.apply {
            model.environment.addOrRemove(
                DockerEnvironmentVariableDefinition.createConnectionVariable(
            ...
            addNodeDeleteListener {
                canvas.remove(this@IgnitionNodeConnector)
            }
        ...
        to.apply {
            model.addServiceModelChangeListener(hostNameChangeListener)
    ...
    private fun deleteConnection() {
        ...
        to.model.removeServiceModelChangeListener(hostNameChangeListener)
    ...
    private fun drawLine(g: Graphics2D) {
        ...
        val exitPoint = allPoints.find {
            !from.bounds.contains(
                it.convert(this, SwingUtilities.getUnwrappedParent(this)),
            )
        } ?: return

        val canvasPoint = exitPoint.convert(this, SwingUtilities.getUnwrappedParent(this))
    ...
    private fun drawArrowIndicator(g: Graphics2D) {
        ...
        val canvasPoint = arrowPoint.convert(this, SwingUtilities.getUnwrappedParent(this))
        ...
        val toCoord = to.location.convert(SwingUtilities.getUnwrappedParent(to), this)
